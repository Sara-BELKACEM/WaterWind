    ...
    <section className="py-20">
      <Container className="grid gap-8 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
        <motion.div initial={{ opacity: 0, scale: 0.96 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true, amount: 0.3 }}>
          <Card className="rounded-[32px] bg-gradient-to-br from-sky-50 via-white to-emerald-50 p-8 shadow-[0_18px_60px_rgba(15,23,42,0.08)]">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="rounded-[24px] bg-white p-6 shadow-sm">
                ...
                <p className="mt-4 text-2xl font-semibold">Sustainable</p>
              </div>
              <div className="rounded-[24px] bg-white p-6 shadow-sm sm:col-span-2">
                <Sparkles size={28} className="text-sky-500" />
                <p className="mt-4 text-2xl font-semibold">Field-ready water intelligence</p>
              </div>
            </div>
          </Card>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, x: 18 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, amount: 0.25 }} className="space-y-4">
          ...
          <div className="space-y-3">
            {checklist.map((item) => (
              <div key={item} className="flex items-center gap-3 rounded-[20px] bg-white p-4 shadow-sm">
